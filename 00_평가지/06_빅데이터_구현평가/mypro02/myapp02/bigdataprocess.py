...
def make_water_fig():
    ...
    df_water = df_water.sort_values(by='ph')
    
    font_name = font_manager.FontProperties(fname='C:\\Windows\\Fonts\\NanumBarunGothic.ttf').get_name() 
    rc('font', family=font_name)
    plt.cla()
    sns.set_style('whitegrid')
    ax = sns.barplot(x='ph', y='구군', data=df_water.sort_values(by='ph'))
    
    for initialx, p in enumerate(ax.patches):
        ax.text(0.2, initialx+p.get_height()/8 ,"{:.3f}".format(p.get_width()))
        
    plt.savefig('./static/images/water_fig.png', dpi=100)
